...
def get_discrete_state3(processed_state):
    image_height = processed_state.shape[0]
    image_width = processed_state.shape[1]
    tile_height = 15
    tile_width = 15
...
def process(state, variant=0, mode=0):
    if mode == 0:
        state = state[...,::-1]
        ...
        if variant == 1:
            return get_discrete_state2(result)
        if variant == 2:
            return get_discrete_state3(result)
    if mode == 1:
        img5 = cv2.cvtColor(state, cv2.COLOR_BGR2GRAY)
        result = get_mario(img5) + get_floor(img5) + get_coinblocks(img5) + get_pipes(img5) + get_koopas(img5) + get_flag(img5)
        #show_grid(result)
...
if __name__ == "__main__":
    for i in range(10, 36):
        img1 = cv2.imread(f"test{i}.png")
        cv2.imshow("original", img1)
        cv2.waitKey()
        processed = process(img1, variant=2, mode=0)
